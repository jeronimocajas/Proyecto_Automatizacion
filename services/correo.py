    ...
    for ruta in archivos_adjuntos:
        ...
        if path.exists():
            with open(path, "rb") as f:
                ...
                adjunto.add_header("Content-Disposition", "attachment", filename=path.name)
                msg.attach(adjunto)

    try:
        print(f"Intentando enviar correo a: {destinatario}")
        print(f"SMTP: {settings.SMTP_HOST}:{settings.SMTP_PORT} usuario:{settings.SMTP_USER}")
        if settings.SMTP_TLS:
            servidor = smtplib.SMTP(settings.SMTP_HOST, settings.SMTP_PORT)
            servidor.starttls()
        else:
            servidor = smtplib.SMTP_SSL(settings.SMTP_HOST, settings.SMTP_PORT)
...
def enviar_notificacion_rechazo(
    ...
    msg["Subject"] = f"[CECAR] Tu solicitud de {tipo_auxilio} requiere correcciones"
    ...
    try:
        print(f"Intentando enviar correo de rechazo a: {correo_estudiante}")
        if settings.SMTP_TLS:
            servidor = smtplib.SMTP(settings.SMTP_HOST, settings.SMTP_PORT)
            servidor.starttls()
        else:
            servidor = smtplib.SMTP_SSL(settings.SMTP_HOST, settings.SMTP_PORT)
        servidor.login(settings.SMTP_USER, settings.SMTP_PASSWORD)
        servidor.sendmail(settings.SMTP_USER, correo_estudiante, msg.as_string())
        servidor.quit()
        print(f"CORREO RECHAZO ENVIADO OK a {correo_estudiante}")
        return True
    except Exception as e:
        print(f"ERROR SMTP RECHAZO: {type(e).__name__}: {e}")
        return False
...
def enviar_codigo_verificacion(correo_destino: str, codigo: str) -> bool:
    ...
    msg["Subject"] = "[CECAR] Codigo de verificacion - Auxilios Economicos"
    ...
    try:
        print(f"Intentando enviar codigo a: {correo_destino}")
        if settings.SMTP_TLS:
            servidor = smtplib.SMTP(settings.SMTP_HOST, settings.SMTP_PORT)
            servidor.starttls()
        else:
            servidor = smtplib.SMTP_SSL(settings.SMTP_HOST, settings.SMTP_PORT)
        servidor.login(settings.SMTP_USER, settings.SMTP_PASSWORD)
        servidor.sendmail(settings.SMTP_USER, correo_destino, msg.as_string())
        servidor.quit()
        print(f"CODIGO ENVIADO OK a {correo_destino}")
        return True
    except Exception as e:
        print(f"ERROR SMTP CODIGO: {type(e).__name__}: {e}")
        return False